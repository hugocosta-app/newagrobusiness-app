...
from app.models.user import User # Needed if filtering orders by seller
from flask_login import login_required, current_user
from datetime import datetime
import io # For TXT report
...
def create_order():
    if request.method == "POST":
        ...
        if not all([customer_name, customer_cnpj, delivery_address, product_ids, quantities]):
            ...
            # Repopulate products for the form
        ...
        try:
            for i in range(len(product_ids)):
                ...
                discount = float(discounts[i] or 0.0) # Default discount to 0 if empty
                ...
                order_item = OrderItem(
                    order=new_order, # Associate with the order being created
            ...
            new_order.calculate_total() # This method needs to be implemented in the Order model

            db.session.commit()
            flash("Pedido criado com sucesso!", "success")
            # Redirect to order details or list, maybe offer TXT download immediately
...
def generate_txt_report(order_id):
    ...
    output.write("========================================\n")
    output.write(f"Data do Pedido: {order.order_date.strftime("%d/%m/%Y %H:%M:%S")}\n")
    ...
    # Simple split for address lines, adjust if needed
    ...
    for item in order.items:
        ...
        output.write("{:<5} {:<30} {:>10} {:>10.2f} {:>10.1f} {:>12.2f}\n".format(
            item.product_id,
            item.product.name[:30], # Truncate name if too long
            item.quantity,
            item.price_at_order,
